...
public class ServerRunnable implements Runnable {
	...
	public void run() {
		...
		try {
            ...
            if (!login(name)) {
	            ...
	            // The user was able to login so add him to our agents list. Also make sure the servers get a graphic reminder
	            System.out.println(name +  " Has Connected to the Server");
	            theServer.addWriter(out);
	            theServer.addAgent(new Agent(name,true));
	            
	
	            //Okay, from now on we will probably get some requests by this connected agent. We'll handle them right here.
	            while (true) {
	...
	/**
	 * Login is a function which simply sends false to a connected socket if they are not allowed to login
	 * @param name
	 */
	...
	/**
	 * Send a list of agents to a client which requested them. We send with it name and boolean online
	 * @param out the PrintWriter to send it to
	 */
